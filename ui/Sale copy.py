...
class Sale:
    ...
    def _init_settings_and_data(
        ...
        self.added_tax_from_edit = 0
        self.added_discount_from_edit = 0
...
"""
    ...
    def _build_payment_dialog_actions(self, dialog, subtotal, discount, tax, total):
        ...
        CTkButton(
            ...
            fg_color="#1676A3",
            hover_color="#115B7E",
            command=lambda: self._print_invoice_with_data(subtotal, total),
        ).pack(side="right", padx=10)
    ...
    def _show_price_edit_dialog(self, product):
        ...
        price_entry.focus()
        key_shortcut(price_entry, "<Return>", lambda: self._save_price_edit(dialog, product, price_var))
    ...
    def _save_price_edit(self, dialog, product, price_var):
        ...
        if new_price < product["price"]:
            ...
            self.added_discount_from_edit += product["price"] - new_price
            self.discount_value_var.set(self.added_discount_from_edit)
            ...
            self.added_tax_from_edit += new_price - product["price"]
            self.tax_var.set(self.added_tax_from_edit)
        else:
    ...
    def _print_invoice_with_data(self, subtotal, total):
        """Print invoice with current sale data."""
        try:
            sale_data = self._prepare_sale_data(
                subtotal,
                self._calculate_discount(float(self.discount_value_var.get())),
                self._calculate_tax(float(self.tax_var.get())),
                total,
    ...
    def _print_invoice(self, sale_data):
        ...
        def run_print():
            try:
                ...
                self.root.after(
                    0, lambda: messagebox.showwarning("تحذير", error_message)
                )
